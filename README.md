# 🎮 Neowiz Web Publishing (Personal Project)

반응형 웹 퍼블리싱을 중심으로 제작한 프로젝트입니다.  
디자인을 기반으로 정확한 마크업과 UI 구현에 집중했습니다.

---

## 🔗 Live Site
👉 [(네오위즈 클론코딩 바로가기)](https://cook2ie.github.io/neowiz/)

---

## 🧩 Overview
- 반응형 웹 구현 (PC / Tablet / Mobile)
- 시멘틱 마크업 기반 구조 설계
- UI 컴포넌트 단위 퍼블리싱

---

## 🛠 Tech Stack
- HTML / CSS / JavaScript
- jQuery

---

## 👨‍💻 Key Points
- 반응형 레이아웃 구현
- 클래스 구조 정리 및 재사용성 고려
- 디자인 시안 기반 퍼블리싱

---

## 💡 What I Learned
- position 기반으로 구성된 레이아웃이 고해상도 환경에서 깨지는 문제를 발견
- 단순한 위치 이동 방식이 아닌, 구조적인 정렬과 max-width를 활용해 레이아웃을 재설계
- 다양한 디바이스와 해상도에 대응하는 유연한 UI 설계의 중요성을 체감

---

## 📸 Preview
<img width="3807" height="1895" alt="image" src="https://github.com/user-attachments/assets/02e9fc4e-caf7-4f5e-807a-6d1601933742" />


